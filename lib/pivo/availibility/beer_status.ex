defmodule Pivo.Availibility.BeerStatus do
  ...
  def changeset(beer_status, attrs) do
    ...
    |> validate_format(:comment, ~r/^(?!.*<[^>]+>).*$/,
      message: "comment cannot contain HTML tags"
    )
    |> validate_format(:comment, ~r/^(?!.*<script).*$/,
      message: "comment cannot contain <script> tags"
    )
    |> validate_format(:comment, ~r/^(?!.*javascript:).*$/,
      message: "comment cannot contain JavaScript injection"
    )
